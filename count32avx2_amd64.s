...
// 32 bit positional population count using AVX-2.
...
// func count32avx2(counts *[32]int, buf []uint16)
TEXT ·count32avx2(SB),NOSPLIT,$0-32
	MOVQ counts+0(FP), DI
	MOVQ buf_base+8(FP), SI		// SI = &buf[0]
	MOVQ buf_len+16(FP), CX		// CX = len(buf)

	// some of the counters, 4 in each register
	VMOVDQU 0*32(DI), Y8
	VMOVDQU 1*32(DI), Y9
	VMOVDQU 4*32(DI), Y10
	VMOVDQU 5*32(DI), Y11

	SUBQ $15*32/4, CX			// pre-decrement CX
	JL end15
...
#define D	75
	...
	VPUNPCKLDQ Y4, Y6, Y0
	VPUNPCKHDQ Y4, Y6, Y1	// Y0, Y1 = fedc:ba98:7654:3210:fedc:ba98:7654:3210
	VPERM2I128 $0x20, Y1, Y0, Y3
	VPERM2I128 $0x31, Y1, Y0, Y4
	VPADDB Y4, Y3, Y0

	// load remaining counters
	VMOVDQU 2*32(DI), Y4
	VMOVDQU 3*32(DI), Y5
	VMOVDQU 6*32(DI), Y6
	VMOVDQU 7*32(DI), Y7

	// counters in Y8:Y9:Y4:Y5:Y10:Y11:Y6:Y7

        // add to counters
	VEXTRACTI128 $1, Y0, X2	// perform two summation chains in parallel
	VPMOVZXBQ X0, Y1
	VPMOVZXBQ X2, Y3
	VPSRLDQ $4, X0, X0
	VPSRLDQ $4, X2, X2
...
end15:	SUBQ $1-15*(32/4), CX		// undo last subtraction and
	JL end1				// pre-subtract 1 dword from CX
	...
	// scalar tail: process four bytes at a time
	VPXOR X0, X0, X0		// X1: 16 byte sized counters
	VPBROADCASTQ magic<>+0(SB), Y2	// X2: mask of bits positions
	VMOVD magic<>+16(SB), X3	// X3: permutation vector to broadcast
	VPUNPCKLBW X3, X3, X3		// one byte into each qword of the register
	VPUNPCKLWD X3, X3, X3
	VPUNPCKHDQ X3, X3, X1
	VPUNPCKLDQ X3, X3, X3
	VINSERTI128 $1, X1, Y3, Y3

scalar:	VPBROADCASTD (SI), Y1		// load four bytes from the buffer
	ADDQ $4, SI			// advance buffer past the loaded bytes
	VPSHUFB Y3, Y1, Y1		// shuffle them around as needed
	VPAND Y2, Y1, Y1		// mask out the desired bytes
