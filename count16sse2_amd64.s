...
TEXT ·count16sse2(SB),NOSPLIT,$0-32
	MOVQ counts+0(FP), DI
	MOVQ buf_base+8(FP), SI		// SI = &buf[0]
	MOVQ buf_len+16(FP), CX		// CX = len(buf)

	// keep last 8 counters in registers
	...
	SUBQ $15*16/2, CX		// pre-decrement CX
	JL end15
...
#define D	75
	...
	MOVOU 0*8(DI), X5	// load first 8 counters
	MOVOU 2*8(DI), X6
	MOVOU 4*8(DI), X7
	MOVOU 6*8(DI), X8
...
end15:	SUBQ $(2/2)-15*(16/2), CX	// undo last subtraction and
...
scalar:	MOVWLZX (SI), AX		// load two bytes from the buffer
	ADDQ $2, SI			// advance buffer past the loaded bytes
	MOVD AX, X1			// into an SSE register
	PUNPCKLBW X1, X1
	PUNPCKLWL X1, X1		// shuffle them around as needed
	PSHUFL $0x50, X1, X1		// high qword gets the high bytes, low qword low byte
