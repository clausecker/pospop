// Copyright (c) 2020 Robert Clausecker <fuz@fuz.su>

#include "textflag.h"

// 32 bit positional population count using SSE2.
// Processes 240 bytes at a time using a 15-fold
// carry-save-adder reduction.
// Required feature flags: SSE2.

// B:A = A+B+C, D used for scratch space
#define CSA(A, B, C, D) \
	MOVOA A, D \
	PAND B, D \
	PXOR B, A \
	MOVOA A, B \
	PAND C, B \
	PXOR C, A \
	POR D, B

// magic transposition constants, comparison constants
DATA magic<>+ 0(SB)/8, $0x0f0f0f0f0f0f0f0f
DATA magic<>+ 8(SB)/8, $0x00aa00aa00aa00aa
DATA magic<>+16(SB)/8, $0x0000cccc0000cccc
DATA magic<>+24(SB)/8, $0x8040201008040201
GLOBL magic<>(SB), RODATA|NOPTR, $32

// pseudo-transpose the 4x8 bit matrices in Y.  Transforms
// Y = D7D6D5D4 D3D2D1D0 C7C6C5C4 C3C2C1C0 B7B6B5B4 B3B2B1B0 A7A6A5A4 A3A2A1A0
// to  D7C7B7A7 D3C3B3A3 D6C6B6A6 D2C2B2A2 D5C5B5A5 D1C1B1A1 D4C4B4A4 D0C0B0A0
// requires magic constants 0x00aa00aa in Y14 and 0x0000cccc in Y15
#define TRANSPOSE(Y) \
	BITPERMUTE(Y, X14, $7) \
	BITPERMUTE(Y, X15, $14)

// swap the bits of Y selected by mask M with those S bits to the left.
// uses X7 as a temporary register
#define BITPERMUTE(Y, M, S) \
	MOVOA Y, X7 \
	PSRLL S, X7 \
	PXOR Y, X7 \
	PAND M, X7 \
	PXOR X7, Y \
	PSLLL S, X7 \
	PXOR X7, Y

// func count32sse2(counts *[16]int, buf []uint16)
TEXT ·count32sse2(SB),NOSPLIT,$0-32
	MOVQ counts+0(FP), DI
	MOVQ buf_base+8(FP), SI		// SI = &buf[0]
	MOVQ buf_len+16(FP), CX		// CX = len(buf)

	SUBQ $15*16/4, CX		// pre-decrement CX
	JL end15

	MOVQ magic<>+ 0(SB), X13	// low nibbles
	MOVQ magic<>+ 8(SB), X14	// for TRANSPOSE
	MOVQ magic<>+16(SB), X15	// for TRANSPOSE

	// broadcast constants to 128 bit
	PUNPCKLLQ X14, X14
	PUNPCKLLQ X15, X15
	PUNPCKLLQ X13, X13

vec15:	MOVOU 0*16(SI), X0		// load 480 bytes from buf
	MOVOU 1*16(SI), X1		// and sum them into Y3:Y2:Y1:Y0
	MOVOU 2*16(SI), X4
	MOVOU 3*16(SI), X2
	MOVOU 4*16(SI), X3
	MOVOU 5*16(SI), X5
	MOVOU 6*16(SI), X6
	CSA(X0, X1, X4, X7)
	MOVOU 7*16(SI), X4
	CSA(X3, X2, X5, X7)
	MOVOU 8*16(SI), X5
	CSA(X0, X3, X6, X7)
	MOVOU 9*16(SI), X6
	CSA(X1, X2, X3, X7)
	MOVOU 10*16(SI), X3
	CSA(X0, X4, X5, X7)
	MOVOU 11*16(SI), X5
	CSA(X0, X3, X6, X7)
	MOVOU 12*16(SI), X6
	CSA(X1, X3, X4, X7)
	MOVOU 13*16(SI), X4
	CSA(X0, X5, X6, X7)
	MOVOU 14*16(SI), X6
	CSA(X0, X4, X6, X7)
	CSA(X1, X4, X5, X7)
	CSA(X2, X3, X4, X7)

	ADDQ $15*16, SI
#define D	90
	PREFETCHT0 (D+ 0)*16(SI)
	PREFETCHT0 (D+ 4)*16(SI)
	PREFETCHT0 (D+ 8)*16(SI)
	PREFETCHT0 (D+12)*16(SI)

	// shuffle registers such that X3:X2:X1:X0 contains dwords
	// of the form 0xDDCCBBAA
	MOVOU X2, X5
	PUNPCKLBW X3, X2		// X2 = DDCCDDCC (lo)
	PUNPCKHBW X3, X5		// X5 = DDCCDDCC (hi)
	MOVOU X0, X3
	PUNPCKLBW X1, X0		// X0 = BBAABBAA (lo)
	PUNPCKHBW X1, X3		// X3 = BBAABBAA (hi)
	MOVOU X0, X1
	PUNPCKLWL X2, X0		// X0 = DDCCBBAA (0)
	PUNPCKHWL X2, X1		// X1 = DDCCBBAA (1)
	MOVOU X3, X2
	PUNPCKLWL X5, X2		// X2 = DDCCBBAA (2)
	PUNPCKHWL X5, X3		// X3 = DDCCBBAA (3)

	// pseudo-transpose the 8x4 bit matrix in each dword
	TRANSPOSE(X0)
	TRANSPOSE(X1)
	TRANSPOSE(X2)
	TRANSPOSE(X3)

	// pull out low nibbles from matrices
	MOVOA X0, X4
	PAND X13, X4
	PSRLL $4, X0

	MOVOU X1, X5
	PAND X13, X5
	PSRLL $4, X1
	PADDB X5, X4

	MOVOU X2, X6
	PAND X13, X6
	PSRLL $4, X2

	MOVOU X3, X7
	PAND X13, X7
	PSRLL $4, X3

	PADDB X7, X6
	PADDB X6, X4		// X4 = ba98:3210:ba98:3210

	// pull out high nibbles from matrices
	PAND X13, X0
	PAND X13, X1
	PADDB X0, X1
	PAND X13, X2
	PAND X13, X3
	PADDB X2, X3
	PADDB X3, X1		// X1 = fedc:7654:fedc:7654

	MOVOA X4, X0
	PUNPCKLLQ X1, X0	// X0 = fedc:ba98:7654:3210 (lo)
	PUNPCKHLQ X1, X4	// X4 = fedc:ba98:7654:3210 (hi)

	PXOR X5, X5		// zero register
	MOVOU 0*8(DI), X6	// load first few counters
	MOVOU 2*8(DI), X7
	MOVOU 4*8(DI), X8
	MOVOU 6*8(DI), X9

	// zero extend byte -> word
	MOVOA X0, X1
	PUNPCKLBW X5, X0
	PUNPCKHBW X5, X1

	// zero extend low half word -> dword
	MOVOA X0, X2
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X2

	// zero extend low dwords -> qwords and add to counters
	MOVOA X0, X3
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X3
	PADDQ X0, X6
	PADDQ X3, X7
	MOVOU X6, 0*8(DI)
	MOVOU X7, 2*8(DI)
	MOVOU 8*8(DI), X6
	MOVOU 10*8(DI), X7

	MOVOA X2, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X8, X0
	PADDQ X9, X2
	MOVOU X0, 4*8(DI)
	MOVOU X2, 6*8(DI)
	MOVOU 12*8(DI), X8
	MOVOU 14*8(DI), X9

	// zero extend high half word -> dword
	MOVOA X1, X0
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X1

	// zero extend high dwords -> qwords and add to counters
	MOVOA X0, X2
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X0, X6
	PADDQ X2, X7
	MOVOU X6, 8*8(DI)
	MOVOU X7, 10*8(DI)
	MOVOU 16*8(DI), X6
	MOVOU 18*8(DI), X7

	MOVOA X1, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X1
	PADDQ X8, X0
	PADDQ X9, X1
	MOVOU X0, 12*8(DI)
	MOVOU X1, 14*8(DI)
	MOVOU 20*8(DI), X8
	MOVOU 22*8(DI), X9

	// zero extend byte -> word
	MOVOA X4, X0
	MOVOA X4, X1
	PUNPCKLBW X5, X0
	PUNPCKHBW X5, X1

	// zero extend low half word -> dword
	MOVOA X0, X2
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X2

	// zero extend low dwords -> qwords and add to counters
	MOVOA X0, X3
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X3
	PADDQ X0, X6
	PADDQ X3, X7
	MOVOU X6, 16*8(DI)
	MOVOU X7, 18*8(DI)
	MOVOU 24*8(DI), X6
	MOVOU 26*8(DI), X7

	MOVOA X2, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X8, X0
	PADDQ X9, X2
	MOVOU X0, 20*8(DI)
	MOVOU X2, 22*8(DI)
	MOVOU 28*8(DI), X8
	MOVOU 30*8(DI), X9

	// zero extend high half word -> dword
	MOVOA X1, X0
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X1

	// zero extend high dwords -> qwords and add to counters
	MOVOA X0, X2
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X0, X6
	PADDQ X2, X7
	MOVOU X6, 24*8(DI)
	MOVOU X7, 26*8(DI)

	MOVOA X1, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X1
	PADDQ X8, X0
	PADDQ X9, X1
	MOVOU X0, 28*8(DI)
	MOVOU X1, 30*8(DI)

	SUBQ $15*(16/4), CX
	JGE vec15			// repeat as long as bytes are left

end15:	SUBQ $(4/4)-15*(16/4), CX	// undo last subtraction and
	JL end1				// pre-subtract 2 byte from CX

	// scalar tail: process four bytes at a time
	PXOR X0, X0			// X0, X4: 32 byte sized counters
	PXOR X4, X4
	MOVQ magic<>+24(SB), X2		// X2: mask of bits positions
	PUNPCKLQDQ X2, X2

scalar:	MOVD (SI), X3			// load four bytes into an SSE register
	ADDQ $4, SI			// advance buffer past the loaded bytes
	PUNPCKLBW X3, X3		// X3 = ----:----:3322:1100
	PUNPCKLWL X3, X3		// X3 = 3333:2222:1111:0000
	MOVOA X3, X1
	PUNPCKHLQ X1, X1		// X1 = 3333:3333:2222:2222
	PUNPCKLLQ X3, X3		// X3 = 1111:1111:0000:0000
	PAND X2, X3			// mask out the desired bytes
	PAND X2, X1
	PCMPEQB X2, X3			// set byte to -1 if corresponding bit set
	PCMPEQB X2, X1
	PSUBB X3, X0			// and subtract from the counters
	PSUBB X1, X4

	SUBQ $1, CX			// decrement counter and loop
	JGE scalar

	PXOR X5, X5		// zero register
	MOVOU 0*8(DI), X6	// load first few counters
	MOVOU 2*8(DI), X7
	MOVOU 4*8(DI), X8
	MOVOU 6*8(DI), X9

	// zero extend byte -> word
	MOVOA X0, X1
	PUNPCKLBW X5, X0
	PUNPCKHBW X5, X1

	// zero extend low half word -> dword
	MOVOA X0, X2
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X2

	// zero extend low dwords -> qwords and add to counters
	MOVOA X0, X3
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X3
	PADDQ X0, X6
	PADDQ X3, X7
	MOVOU X6, 0*8(DI)
	MOVOU X7, 2*8(DI)
	MOVOU 8*8(DI), X6
	MOVOU 10*8(DI), X7

	MOVOA X2, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X8, X0
	PADDQ X9, X2
	MOVOU X0, 4*8(DI)
	MOVOU X2, 6*8(DI)
	MOVOU 12*8(DI), X8
	MOVOU 14*8(DI), X9

	// zero extend high half word -> dword
	MOVOA X1, X0
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X1

	// zero extend high dwords -> qwords and add to counters
	MOVOA X0, X2
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X0, X6
	PADDQ X2, X7
	MOVOU X6, 8*8(DI)
	MOVOU X7, 10*8(DI)
	MOVOU 16*8(DI), X6
	MOVOU 18*8(DI), X7

	MOVOA X1, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X1
	PADDQ X8, X0
	PADDQ X9, X1
	MOVOU X0, 12*8(DI)
	MOVOU X1, 14*8(DI)
	MOVOU 20*8(DI), X8
	MOVOU 22*8(DI), X9

	// zero extend byte -> word
	MOVOA X4, X0
	MOVOA X4, X1
	PUNPCKLBW X5, X0
	PUNPCKHBW X5, X1

	// zero extend low half word -> dword
	MOVOA X0, X2
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X2

	// zero extend low dwords -> qwords and add to counters
	MOVOA X0, X3
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X3
	PADDQ X0, X6
	PADDQ X3, X7
	MOVOU X6, 16*8(DI)
	MOVOU X7, 18*8(DI)
	MOVOU 24*8(DI), X6
	MOVOU 26*8(DI), X7

	MOVOA X2, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X8, X0
	PADDQ X9, X2
	MOVOU X0, 20*8(DI)
	MOVOU X2, 22*8(DI)
	MOVOU 28*8(DI), X8
	MOVOU 30*8(DI), X9

	// zero extend high half word -> dword
	MOVOA X1, X0
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X1

	// zero extend high dwords -> qwords and add to counters
	MOVOA X0, X2
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X2
	PADDQ X0, X6
	PADDQ X2, X7
	MOVOU X6, 24*8(DI)
	MOVOU X7, 26*8(DI)

	MOVOA X1, X0
	PUNPCKLLQ X5, X0
	PUNPCKHLQ X5, X1
	PADDQ X8, X0
	PADDQ X9, X1
	MOVOU X0, 28*8(DI)
	MOVOU X1, 30*8(DI)

	// write remaining counters pack
end1:	RET
