...
TEXT ·count32sse2(SB),NOSPLIT,$0-16
	MOVL counts+0(FP), DI
	MOVL buf_base+4(FP), SI		// SI = &buf[0]
	MOVL buf_len+8(FP), CX		// CX = len(buf)

	SUBL $15*(16/4), CX		// pre-decrement CX
	JL end15
...
#define D	90
	...
	PUNPCKHLQ X1, X4	// X1 = fedc:ba98:7654:3210 (hi)
	...
	// zero extend X0 to X2:X0 and add
	MOVOA X0, X2
	PUNPCKLWL X5, X0
	PUNPCKHWL X5, X2
	PADDL X0, X6
	...
	// zero extend X1 to X2:X1 and add
	MOVOA X1, X2
	PUNPCKLWL X5, X1
	PUNPCKHWL X5, X2
	PADDL X1, X6
	...
	// zero extend X4 to words X1:X4
	MOVOA X4, X1
	PUNPCKLBW X5, X4
	PUNPCKHBW X5, X1

	// zero extend X4 to X2:X4 and add
	MOVOA X4, X2
	PUNPCKLWL X5, X4
	PUNPCKHWL X5, X2
	PADDL X4, X6
...
end15:	SUBL $1-15*(16/4), CX		// undo last subtraction and
	JL end1				// pre-subtract 1 word from CX

	// scalar tail: process four bytes at a time
	PXOR X0, X0			// X0, X4: 16 byte sized counters
	PXOR X4, X4
...
scalar:	MOVD (SI), X1			// load four bytes from the buffer
	ADDL $4, SI			// advance buffer past the loaded bytes
