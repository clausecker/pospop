...
// 32 bit positional population count using AVX2.
...
TEXT ·count32avx2(SB),NOSPLIT,$0-16
	MOVL counts+0(FP), DI
	MOVL buf_base+4(FP), SI		// SI = &buf[0]
	MOVL buf_len+8(FP), CX		// CX = len(buf)

	SUBL $15*(32/4), CX		// pre-decrement CX
	JL end15
...
#define D	75
	...
	VPUNPCKLDQ Y1, Y4, Y0
	VPUNPCKHDQ Y1, Y4, Y4	// Y0, Y4 = fedc:ba98:7654:3210
//	VPERM2I128 $0x20, Y4, Y0, Y3
	BYTE $0xc4
	BYTE $0xe3
	BYTE $0x7d
	BYTE $0x46
...
scalar:	VPBROADCASTD (SI), Y1
	ADDL $4, SI			// advance buffer past the loaded bytes
	VPSHUFB Y3, Y1, Y1		// shuffle high byte into high qword, low byte into low qword
